...
/ddemo         # 分析功能点不确定性，识别需要先验证的 Demo（正式版本时）
/dtask         # 将方案拆解为可执行任务列表
```

> 示例：要做一个「消息通知」功能 → 先 `/dprd` 讨论推送方式和触发条件，确定用 WebSocket 还是轮询时用 `/dadr` 记录决策，方案通过后 `/ddoc` 写产品文档，用 `/ddemo` 识别哪些能力需要先隔离验证，最后 `/dtask` 拆出具体任务。
...
| `/ddemo` | 分析功能点不确定性，输出 Demo 必要性报告 | PRD 通过后、任务拆解前 |
