...
```
/dprd          # 与产品讨论方案，生成 PRD（识别 Demo 需求）
/dadr          # 有架构决策时记录（可选）
/ddoc          # 基于 PRD 正向生成产品文档
/ddemo         # 对每个 Demo 需求生成落地方案（场景 + 工作台 + 验证资产）
/dtask         # Demo 验证通过后，拆解集成任务列表
```

> 示例：要做一个「消息通知」功能 → 先 `/dprd` 讨论推送方式和触发条件，确定用 WebSocket 还是轮询时用 `/dadr` 记录决策，方案通过后 `/ddoc` 写产品文档，用 `/ddemo` 为每个不确定性功能点（如 Prompt 稳定性）生成验证方案，Demo 通过后 `/dtask` 拆出集成任务。
...
| `/ddemo` | 针对单个 Demo，生成完整落地方案（场景演示 + 工作台 UI + 核心验证资产） | PRD 识别出 Demo 需求后 |
...
| 状态 | 含义 | 谁可以操作 |
|------|------|-----------|
| `InDraft` | 需求草稿中，任务标题、任务描述、验收条件、实施步骤均可自由修改 | 人工 + Agent |
...
|------|------|------|
| `id` | 数字 | 从 1 递增，永不复用 |
| `title` | 字符串 | 动词开头的一句话任务标题（做什么） |
| `description` | 字符串 | 任务背景和关键约束（为什么做、边界是什么） |
| `status` | 字符串 | 见上方状态说明 |
