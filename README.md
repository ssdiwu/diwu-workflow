...
| **Skill**（10 个） | 各领域详细知识与操作规范 | Agent 遇到对应场景时通过 `/skill-name` 或 `Read` 按需加载 |
...
| `dvfy` | 规则 | L1-L5 证据优先级、Done 判定、验证充分性 | verification.md |
| `djug` | 规则 | 阶段边界决策、幅度判定、并行 vs 串行 | judgments.md |
| `drec` | 规则 | Session 记录格式、踩坑经验、时间戳规则 | session.md(recording段) + templates.md |
| `darc` | 规则 | Task/Recording 归档、触发检测、验证清单 | 原有（Task#192-195 独立） |
...
由 `/dinit` 初始化到 `.diwu/dsettings.json`，用户可按项目调整。
