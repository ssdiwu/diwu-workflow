...
**Q6 不确定性**：哪些工作结果不可预期？需先跑 Demo 验证；哪些有现成模式可直接集成？
  ...
  > **能力命名约束**：识别 Demo 时，名称必须体现能力而非产品功能——「换一个产品，这个 Demo 还能用吗？」能用 → 命名正确；不能用 → 重新定义，把产品特定部分剥离。
...
- 使用 WebSearch 搜索竞品，关键词示例：`"{产品类型} competitors"`, `"{核心功能} best tools 2026"`
- 整理竞品对比表（产品名称 × 核心功能维度），每格填写具体实现方式
...
- **Demo 需求清单**（功能规格表中存在 Layer 0 时必须输出，放在 §3 末尾）：
  ...
  > 每条 Layer 0 对应一行。Demo 名称用 kebab-case，直接作为 `/ddemo` 的输入参数。名称体现**能力**（换项目能用），不体现产品功能——❌「hera-meeting-citation」→ ✅「prompt-citation-stability」。
