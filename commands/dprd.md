...
**Q6 不确定性**：哪些工作结果不可预期？需先跑 Demo 验证；哪些有现成模式可直接集成？（判断标准和 Demo 命名规范见 diwu-prd Skill）
...
- **业务成功指标**：格式见 diwu-prd Skill §业务指标定义规范
- **失败止损条件**：格式见 diwu-prd Skill §业务指标定义规范
...
- 功能点表格：**必须包含"迭代层次"列，放在功能细节之前作为优先级总览**（迭代层次定义见 diwu-prd Skill §需求优先级）
  ...
  **Layer 0（Demo 验证）**：当功能涉及 LLM/Prompt 或其他结果不可预期的能力时，Layer 1 之前必须有 Layer 0。Layer 0 未通过时，Layer 1 不得开始。（不确定性评估标准见 diwu-prd Skill）

- **Demo 需求清单**（功能规格表中存在 Layer 0 时必须输出，放在 §3 末尾，格式见 diwu-prd Skill §Demo 需求清单格式）

- 每个功能点一个小节，标注所属层次
- **触发阈值表**（涉及"显示/隐藏"类需求时必须输出，格式见 diwu-prd Skill §触发阈值表）
- **验收条件按层次分组**，规范见 diwu-prd Skill §验收条件规范

#### 4. 方案对比
- 对比表格（维度 × 方案 A/B/C），写作规范见 diwu-prd Skill §方案对比方法论
- 推荐策略：**必须包含推荐理由 + 切换条件**
...
- **竞品对比表**：格式见 diwu-prd Skill §竞品分析方法
- **差异化策略**：我们与竞品的核心差异点
- **借鉴点**：竞品中值得参考的设计或功能
...
#### 8. 边界条件与异常处理
- 按角色/层分组（分组定义和写作规范见 diwu-prd Skill §非功能性需求分类）
...
见 diwu-prd Skill §双端落地 Checklist。

##### Demo 阶段约束

见 diwu-prd Skill §Demo 阶段端侧约束。
