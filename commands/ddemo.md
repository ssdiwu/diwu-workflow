...
1. `.doc/README.md` → 了解系统文档全貌，根据覆盖范围描述定向读取相关文件
2. `.doc/adr/README.md` → 了解已有架构决策，确认有无被否决的方案
3. `.doc/prd/README.md` → 找到触发本 Demo 的原始 PRD 文件名，再精读该 PRD 中关于本 Demo 的描述
4. `.doc/demo/README.md` → 确认是否已有同类 Demo 可复用
...
- **LLM/Prompt 类**：Prompt v1 全文（可直接复制运行，不是伪代码）
- **集成/数据流类**：测试 case 矩阵（输入 → 预期输出 → 验证方法）
- **第三方 API 类**：对接测试脚本 + 边界 case 列表
- **UI/交互类**：交互用例矩阵（操作序列 → 预期状态）
...
|------|------------------------|------------|------|
| DEMO-prompt-citation-prd.md | Prompt 能否稳定输出引用标记格式 | 准确率 > 90%，幻觉率 < 5% | 待验证 |
```

**首次创建 README（兼容旧版本）**：
- 如果 `.doc/demo/` 目录存在但 README.md 不存在，先 glob 扫描 `DEMO-*.md` 文件
- 读取每个 Demo 的"核心不确定性"章节（作为验证内容）、"通过标准"章节（提取关键指标），生成完整索引表
- 状态默认为"待验证"，再追加当前新建的 Demo

**日常维护**：
- 新建 Demo → 追加一行
- Demo 验证通过后 → 修改状态为"已通过"
