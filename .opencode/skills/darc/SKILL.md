...
- **条件**：`.diwu/dtask.json` 中 `status` 为 `Done` 或 `Cancelled` 的任务数量 ≥ `task_archive_threshold`
...
2. Task 归档：
   a. 读取 .diwu/dtask.json，筛选 status=Done/Cancelled 的任务
   b. 写入 .diwu/archive/task_archive_YYYY-MM.json（当月归档文件）
   c. 从 dtask.json 中移除已归档任务（保留活跃任务）
3. Recording 归档：
...
5. 验证清单：
   [ ] dtask.json 中无残留的 Done/Cancelled 任务（超出保留阈值的部分）
...
- **不压缩**归档产物（保持原始格式可读）
- **不归档活跃任务**（InProgress/InSpec/InReview/InDraft 保留在 dtask.json）
- **不修改任务 ID**（归档后 ID 不复用，dtask.json 新任务继续递增）
