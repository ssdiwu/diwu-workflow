...
- Step 2 和 Step 3 的 dtask.json 读取可合并为一次

每个新 session 必须先完成以下步骤，然后才能开始实现任务：

### 1. Preflight 检查
- 运行 `.diwu/checks/smoke.sh`（如存在），验证基线环境
- 检查 recording/ 最新 session 文件中是否有未解决的阻塞记录
- 运行 `git status`，确认工作区状态

### 2. 上下文恢复
- **优先读取** `.diwu/continue-here.md`（如存在），了解精确断点位置，读完后删除该文件
- 如果 continue-here.md 不存在，使用 `ls -t .diwu/recording/ | head -2` 读取最新 1-2 个 session 文件，了解最近的工作和注意事项
- 读取 `.diwu/decisions.md`（如存在），了解历史设计决策
- 运行 `git log --oneline -20`，了解最近的代码变更历史

### 3. 归档检查
- 统计 `.diwu/dtask.json` 中 Done/Cancelled 任务数量
- 如果超过 `dsettings.json` 中的 `task_archive_threshold`（默认 20）：
  - 创建 `.diwu/archive/` 目录（如不存在）
  - 将 Done/Cancelled 任务移到 `archive/task_archive_YYYY-MM.json`（当前月份）
  - 更新 `.diwu/dtask.json` 只保留活跃任务
  - 在后续 session 文件中记录归档操作

### 4. 任务选择策略
- 读取 `.diwu/dtask.json`，了解所有任务的当前状态
...
- 否则选择第一个 `status: "InSpec"` 的任务，并检查 blocked_by：
  ...
  - **blocked_by 存在 InReview 且超前未达上限（超前上限见 dsettings.json）** → 可超前实施（标记为超前）
...
### 触发条件
- 用户在讨论想法、需求、功能设计
- `.diwu/dtask.json` 为空或用户明确要求分解任务
- 用户描述了一个较大的功能，需要拆分为可执行步骤

### 分解流程

**需求精炼（任务描述模糊时触发）** `[建议]`：每次只问一个问题，理解目的、约束、成功标准；提出 2-3 个方案并给出推荐，获得确认后再写 dtask.json。

**草案阶段（InDraft）**：提炼可执行功能点 → 拆分为一句话可描述的任务 → 定义验收条件 → 识别依赖关系排序 → 展示分解结果 → 写入 `.diwu/dtask.json`，状态为 `InDraft`。
...
**提交时机**：
- 常规任务：状态到达 Done 时提交
- 超前任务：完成时标记 InReview 并立即提交（超前上限见 dsettings.json），阻塞解除后标记 Done（不再重复提交）

**commit message 格式**：
- 常规：`[Task#N] 任务标题 - completed`
- 超前：`[Task#N] 任务标题 - completed (超前实施 X/5, blocked_by Task#M)`

**提交内容**：代码变更 + `.diwu/dtask.json` + 新 session 文件，同一个 commit。
...
**Coordinator Pattern**：dtask.json 状态更新由主代理负责，子代理通过返回值传回结果。

**超前计数器所有权**：超前计数器由主代理维护，子代理完成超前任务后通过返回值通知主代理。计数器在内存中维护，不持久化到 dtask.json。Session 重启后通过统计 dtask.json 中状态为 InReview 且 blocked_by 非空的任务数量来恢复计数。

**启动仪式**（所有子代理，强约束）：以下步骤由 SubagentStart Hook 自动注入：recording/ 最新 session 文件摘要、dtask.json 中 InProgress 任务的 title/acceptance/steps、`.diwu/decisions.md` 最近 3 条设计决策。
...
**验证方式**：
1. 自动化验证（推荐）：`.diwu/checks/task_<id>_verify.sh`，返回 0 成功 / 非0 失败
...
**基线验证**：`.diwu/checks/smoke.sh` 验证基线环境（依赖/构建/lint）。
...
1. 确保所有代码变更已提交
2. 写入 `.diwu/recording/session-YYYY-MM-DD-HHMMSS.md` 新文件（运行 `date '+%Y-%m-%d %H:%M:%S'` 获取真实时间戳），记录：Session 标题、处理的任务及状态、验收验证结果、下次应该做什么
3. 如有重大设计决策，追加到 `.diwu/decisions.md`（格式：DEC-NNN 日期 标题、背景、决策、备选方案、理由）
4. 确保 `.diwu/dtask.json` 反映最新的任务状态
