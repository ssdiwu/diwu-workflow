...
| Temporal     | opencode.json 的 version 字段必须在功能变更提交时同步更新，不得事后单独提交                        | 每个功能 commit 的 diff 包含 opencode.json version 变更             |
| Cross-platform | 插件中的路径引用必须兼容 macOS/Linux；禁止使用 Windows 路径分隔符                          | 路径中无 `\` 分隔符                                              |
...
|----------|----------------------------------------|----------------------------------|--------------------------|
| Domain   | commands/, skills/, references/        | OpenCode 宿主环境                | opencode.json 描述注册     |
| Platform | .opencode/ 插件描述                    | 用户的 ~/.config/opencode/ 目录   | 插件安装机制               |
| Data     | .opencode/skills/（技能源文件）         | 用户项目的 .diwu/（生成产物）      | /dinit 命令触发复制        |
...
Source of Truth: `opencode.json`（版本、命令列表、元数据）
生成产物链: `.opencode/skills/*` → 用户项目 `.diwu/`（通过 /dinit）
