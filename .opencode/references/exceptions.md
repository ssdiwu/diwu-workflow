...
用户用 `<private>...</private>` 包裹的内容（密码、密钥、个人敏感信息）不得写入 session 文件和 dtask.json。该内容仅在当前对话上下文中使用，不得持久化。
...
**禁止**:
- 提交 git commit
- 将 `.diwu/dtask.json` 状态设为 Done
- 假装任务已完成

**必须**:
- 在 session 文件中记录当前进度和阻塞原因
- 将 `.diwu/dtask.json` 任务状态退回 **InSpec**(不是 InDraft)
