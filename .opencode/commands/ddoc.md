...
**小范围**：直接进入阶段 1 执行，不写 dtask.json。

**大范围**：将探索+写作任务拆入 `.diwu/dtask.json`（status: InDraft），提示用户确认后，按工作流执行。任务结构：
...
**首次创建 README（兼容旧版本）**：
- 如果 `.doc/` 目录存在但 README.md 不存在，先 glob 扫描所有 `.md` 文件（排除 README.md 本身）
- 读取每个文件的首个一级标题（作为覆盖范围描述），提取文件修改时间，生成完整索引表
- 再追加当前新建的文档
